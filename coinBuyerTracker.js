...
const SOCKETURL = "wss://warmhearted-falling-sheet.avalanche-mainnet.quiknode.pro/49e20ec9d82676938bfcada1b6ee10a9077abac4/ext/bc/C/ws/"
...
Buyer:  <a href='https://avascan.info/blockchain/c/address/${senderAddress}'>${sender}</a> (${firstBuyers[senderAddress.toLowerCase()]?.amount ?? "-"} contract hits)

🏷️ <b>${data.name} (${data.symbol})</b>

<b>Token Details:</b>
⛓️ Chain: ETH
🏷️ Address: <a href='https://avascan.info/blockchain/c/address/${data.id.token}'>${data.id.token.substring(0, 2)}...${data.id.token.substring(data.id.token.length - 2)}</a>
...
📊 View Chart:
<a href='https://www.dextools.io/app/en/avalanche/pair-explorer/${data.id.pair}'>DexTools</a> | <a href='https://dexscreener.com/avalanche/${data.id.token}'>DexScreener</a>
...
<a href='https://t.me/BananaGunSniper_bot?start=snp_Otto_${data.id.token}'>Banana Gun</a> | <a href='https://t.me/unibotsniper_bot?start=OttoBots-${data.id.token}'>UniBot</a> | <a href='https://t.me/MaestroSniperBot?start=${data.id.token}-ottobots'>Maestro</a> | <a href='https://t.me/MaestroProBot?start=${data.id.token}-ottobots'>Maestro Pro</a>
    ...
    if(stablecoins.includes(data.symbol)){
        console.log("Stable coin: " + data.symbol)
        return;
    }
...
async function getInformation(pairAddress){
    ...
    const response = await client.get(`https://www.dextools.io/shared/data/pair?address=${pairAddress}&chain=avalanche&audit=true&locks=true`, {
        headers: {
            "Referer": "https://www.dextools.io/app/en/avalanche/pair-explorer/" + pairAddress,
...
async function startMonitoring(trackedWallets){
    const subscription = web3Socket.eth.subscribe("newBlockHeaders").on("data", async (block) => {
        try{
            let data;
            try{
                data = await web3.eth.getBlock(block.hash)
            }catch(e){
                try{
